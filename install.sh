...
# ========================================
# HOSTLIST & EXCLUSIONS DISABLED (FIX #4)
# ========================================
# DISABLED in v2.6 - only handshake obfuscation
# No auto.list, user.list, or exclude.list reading
# NFQWS obfuscates ALL UDP 443 traffic automatically
NFQWS_EXTRA_ARGS=""
...
# 
# Kept for reference only. NFQWS now obfuscates all UDP 443 traffic
# without reading any domain lists (hostlist is disabled).
#
# If you need selective obfuscation in the future, modify nfqws.conf
# and uncomment: NFQWS_EXTRA_ARGS="--hostlist=/opt/etc/nfqws/user.list"
EOF
...
# DISABLED IN v2.6 - This file is NOT read by NFQWS
#
# Kept for reference only. All UDP 443 traffic is obfuscated
# without any exclusion logic.
EOF
...
###############################################################################
# WireGuard Handshake Restore (Ground-Zerro integration) - FIX #5 & #6
###############################################################################
...
start() {
    # FIX #5: Check if process already running (prevent duplicates)
    if [ -f "$PID_FILE" ]; then
...
start_wireguard_restore() {
    ...
    # Start the daemon (immediately returns, process runs in background)
...
main() {
    ...
    # FIX #6: Start WG Restore and return immediately (don't wait)
    ...
    echo "  ✓ Fix #4: auto.list disabled (only handshake obfuscation, NFQWS_EXTRA_ARGS=\"\")"
    echo "  ✓ Fix #5: WG Restore single process (PID check prevents duplicates)"
    echo "  ✓ Fix #6: Installation log completes immediately (WG Restore in background)"
    echo ""
