...
# ========================================
# WARP UDP 443 OBFUSCATION (CRITICAL FIX)
# ========================================
...
# ========================================
# HOSTLIST & EXCLUSIONS (WARP-ONLY MODE)
# ========================================
# user.list: Domains to obfuscate (empty for WARP-only)
# exclude.list: Domains to skip obfuscation
...
EOF
    ...
    log_success "NFQWS config v2.0 created (WARP-only mode)"
}
...
# WARP domains list (WARP-only mode - stays empty)
# This file is referenced in nfqws.conf via --hostlist
# 
# For WARP-only obfuscation, this list stays empty.
# NFQWS will obfuscate all UDP 443 traffic automatically.
#
# If you add domains here, they will ALSO be obfuscated.
# Example:
# example.com
# api.example.org
EOF
    
    log_success "user.list created (empty for WARP-only mode)"
}
...
# Domains to EXCLUDE from obfuscation
# Leave empty for WARP-only mode (obfuscate everything on UDP 443)
#
# Examples if you need to exclude:
# google.com
# youtube.com
# facebook.com
EOF
...
start_wireguard_restore() {
    log_info "Starting WireGuard restore daemon..."
    ...
    if /opt/etc/init.d/S99wg-restore start 2>&1 | tee -a "$LOG_DIR/install.log"; then
        log_success "WireGuard restore daemon started"
        sleep 1
        return 0
    else
...
main() {
    ...
    log_success "User list: $INSTALL_DIR/user.list"
    ...
    echo "  3. Test: ping through WireGuard interface"
    echo "  4. Monitor logs for DPI bypass success"
    echo ""
